...
Deletion workflow identifies subject, verifies requester, freezes conflicting processing, enumerates database/objects/analytics/observability/providers, deletes or irreversibly anonymizes, records minimal compliance proof, and completes within **15 business days** of a verified request (reference value only -- **must be confirmed by the project's Brazilian counsel before production**, per this document's own opening disclaimer; the automatic `audit_logs` trigger described in `docs/database/schema.sql` records `occurred_at` for the actual deletion/anonymization event, which is what this deadline is measured against). Backups expire naturally; deleted subjects are suppressed from restore reactivation through tombstones until backup window passes.
