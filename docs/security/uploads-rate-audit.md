...
| concurrent AI runs | 1/conversation, 2/visitor, 5/auth user; tenant/global breakers |
| SSE | 2/conversation, 5/IP; idle heartbeat 20 s, max run 120 s |
| website analysis | 3/visitor/day, 20/tenant/day |
...
Use Cloudflare coarse IP/bot controls and Redis sliding/token buckets keyed by tenant+actor+route; hash IP with rotating salt. If Redis is down, use conservative in-process/IP edge limit and emit degraded metric. `Retry-After` required.

## Audit

Audit meaningful writes, auth/session/security events, consent, private/privileged reads, tool policy/confirmation/execution, prompt/agent/knowledge publish, flags/settings, integrations/secrets, staff takeover, export/deletion. Record tenant, timestamp, actor type/ID, action, resource, request/correlation, redacted before/after, tool/agent/model/version, result and IP hash. Never record secret values, full tokens, payment data or duplicate full conversation unnecessarily.
