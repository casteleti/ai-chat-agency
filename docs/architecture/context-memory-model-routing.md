...
2. Published tenant brand/role prompt and capability manifest.
3. Identity, tenant, consent, locale, channel, current page and journey state.
...
| Organization | tenant configuration/brand/process | while active | tenant-only |
...
Router inputs: task type, risk, context size, modality, language, latency target, provider health, tenant budget, and eval-approved model matrix. It cannot route a high-risk action to a model lacking required structured/tool behavior. Provider fallback must pass the same schemas and task eval thresholds. Record route reason, model/provider/config, tokens, cache, latency, cost, prompt and agent version.
