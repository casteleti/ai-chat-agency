...
11. Foreign conversation/resource IDs from another session injected into routes, RAG and action tokens → consistent not-found/forbidden and audit.
...
Pass: error <1% excluding injected/provider-defined failures; no duplicate important writes; API p95/stream/RAG budgets met; DB CPU <70% sustained and connections <80%; queue lag <60 s normal/<5 min after spike; memory stable (<10% growth after warmup); graceful limit responses; no result corruption.
