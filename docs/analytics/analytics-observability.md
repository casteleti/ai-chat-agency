...
- AI: TTFT/latency/error/fallback/schema failure, tokens/cost by tenant/task/model, tool denial/failure, RAG latency/zero-result/citation, eval trend.
- Product: starts, MBC, qualification, meeting/handoff, abandonment, support resolution/escalation by locale/device/source.
- Security: auth failures, rate/Turnstile, injection detections, forbidden tool attempts, cross-tenant canary, upload/SSRF blocks.

Alert examples: API 5xx >2% 5 min; p95 ack >800 ms 10 min; model errors >10%; schema invalid >1%; queue oldest >5 min; dead letter >0 important queue; daily AI cost 80/100% budget; any tenant isolation canary or unauthorized tool execution pages immediately.
