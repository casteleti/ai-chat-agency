...
3. Contain: disable flag/tool/integration, revoke sessions/tokens, isolate host/tenant, WAF rule or maintenance mode according to scope.
4. Determine data/tenants/actions/time/source; query audit with least exposure. Rotate affected secrets using rotation runbook.
5. Eradicate vulnerability, patch/test tenant/auth/tool boundaries, restore from known-good artifacts.
...
Any cross-tenant leak, auth bypass, secret exposure or unauthorized privileged tool execution is Critical until disproven.
